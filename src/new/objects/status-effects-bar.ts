...
export default class StatusEffectsBar extends PIXI.Graphics {
  ...
  constructor(private statusEffectsManager: StatusEffectManager) {
    super();
    this.statusEffectsManager.emitter.on('onEffectStarted', effect => this.addStatusIcon(effect))
    this.statusEffectsManager.emitter.on('onEffectEnded', effect => this.onEffectEnded(effect))

  }

  public addStatusIcon(effect: StatusEffect) {
    const icon = new StatusEffectIcon(effect);
    icon.position.set(icon._width * 1.25 * this.icons.length, 0);
    this.addChild(icon);
